...
import {fetchPlayer} from "./player/fetch-player.js";
...
import {trophyquestAppInsert} from "./postgres/trophyquest-app-insert";
import {OldPlayedGame} from "./models/old-played-game";
import {OldPlayedEdition} from "./models/old-played-edition";
import {OldPlayedTrophySuite} from "./models/old-played-trophy-suite";
...
async function runRefresher(): Promise<void> {
    ...
    try {
        ...
        const playedGameList: OldPlayedGame[] = [];
        const playedEditionList: OldPlayedEdition[] = [];
        const playedTrophySuiteList: OldPlayedTrophySuite[] = [];
        ...
        for (let profile of profiles) {
            const player: Player = await fetchPlayer(auth, profile.pseudo);
