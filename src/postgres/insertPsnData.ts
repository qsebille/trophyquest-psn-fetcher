...
export async function insertPsnData(
    ...
    try {
        ...
        console.info(`[PSN] Upserted ${userResponse.rowsInserted} users into postgres database`);
        console.info(`[PSN] Inserted ${titlesResponse.rowsInserted} titles into postgres database ${titlesResponse.rowsIgnored > 0 ? `(${titlesResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[PSN] Inserted ${playedTitlesResponse.rowsInserted} played titles into postgres database`);
        console.info(`[PSN] Inserted ${trophySetsResponse.rowsInserted} trophy sets into postgres database ${trophySetsResponse.rowsIgnored > 0 ? `(${trophySetsResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[PSN] Inserted ${titlesTrophySetResponse.rowsInserted} titles / trophy sets links into postgres database ${titlesTrophySetResponse.rowsIgnored > 0 ? `(${titlesTrophySetResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[PSN] Inserted ${trophyResponse.rowsInserted} trophies into postgres database ${trophyResponse.rowsIgnored > 0 ? `(${trophyResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[PSN] Inserted ${earnedTrophyResponse.rowsInserted} earned trophies into postgres database ${earnedTrophyResponse.rowsIgnored > 0 ? `(${earnedTrophyResponse.rowsIgnored} ignored)` : ''}`);
    } catch (err) {
