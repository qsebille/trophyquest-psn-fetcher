...
export async function insertPsnData(
    ...
    try {
        ...
        const titleResponse: InsertQueryResult = await insertPsnTitles(client, data.titles);
        ...
        console.info(`Postgres: Inserted ${titleResponse.rowsInserted} lines into psn.title table ${titleResponse.rowsIgnored > 0 ? `(${titleResponse.rowsIgnored} ignored)` : ''}`);
