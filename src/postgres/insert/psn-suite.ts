...
import {TrophyQuestSuite} from "../../trophyquest/trophy-suite";

async function insertBatchWithGameIds(
    client: PoolClient,
    batch: TrophyQuestSuite[],
) {
}
...
export async function insertSuites(
    ...
    if (suites.length === 0) {
        console.warn("🟡 No data to insert into app.psn_suite table.");
    ...
    for (let i = 0; i < suites.length; i += batchSize) {
        ...
        const insert = await client.query(`
            INSERT INTO app.psn_suite (id, name, psn_image_url, platforms)
            VALUES
            ${placeholders}
        ON CONFLICT (id)
            DO NOTHING
