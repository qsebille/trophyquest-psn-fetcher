...
export async function insertAppData(
    ...
    try {
        ...
        console.info(`[APP] Upserted ${playerResponse.rowsInserted} players into postgres database`);
        console.info(`[APP] Inserted ${gameResponse.rowsInserted} games into postgres database ${gameResponse.rowsIgnored > 0 ? `(${gameResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[APP] Inserted ${collectionResponse.rowsInserted} trophy collections into postgres database ${collectionResponse.rowsIgnored > 0 ? `(${collectionResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[APP] Inserted ${trophyResponse.rowsInserted} trophies into postgres database ${trophyResponse.rowsIgnored > 0 ? `(${trophyResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[APP] Inserted ${playedGameResponse.rowsInserted} played games into postgres database`);
        console.info(`[APP] Inserted ${playedTrophyCollectionResponse.rowsInserted} played trophy collections into postgres database ${playedTrophyCollectionResponse.rowsIgnored > 0 ? `(${playedTrophyCollectionResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`[APP] Inserted ${earnedTrophyResponse.rowsInserted} earned trophies into postgres database ${earnedTrophyResponse.rowsIgnored > 0 ? `(${earnedTrophyResponse.rowsIgnored} ignored)` : ''}`);
    } catch (err) {
