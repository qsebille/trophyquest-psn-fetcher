...
export async function insertAppData(
    ...
    try {
        ...
        console.info(`Upserted ${playerResponse.rowsInserted} players into postgres database`);
        console.info(`Inserted ${gameResponse.rowsInserted} games into postgres database ${gameResponse.rowsIgnored > 0 ? `(${gameResponse.rowsIgnored} ignored)` : ''}`);
        console.info(`Inserted ${collectionResponse.rowsInserted} trophy collections into postgres database ${collectionResponse.rowsIgnored > 0 ? `(${collectionResponse.rowsIgnored} ignored)` : ''}`);
        ...
        console.info(`Inserted ${playedGameResponse.rowsInserted} played games into postgres database`);
        console.info(`Inserted ${playedTrophyCollectionResponse.rowsInserted} played trophy collections into postgres database ${playedTrophyCollectionResponse.rowsIgnored > 0 ? `(${playedTrophyCollectionResponse.rowsIgnored} ignored)` : ''}`);
